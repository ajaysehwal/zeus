...
export default class SyncStore {
        ...
        connectivityStore.onReconnect(() => {
            if (
                ...
                void this.startSyncing().catch(() => {
                    // LND may be stopped; avoid unhandled rejection
                });
            }
        });
    }
    ...
    public startSyncing = async () => {
        ...
        try {
            ...
            // setSyncInfo should not throw; guard anyway
        }
            ...
            .then((data: any) => {
                if (data.recovery_mode && !data.recovery_finished) {
                    void this.startRecovering().catch(() => {
                        // LND may stop while recovery polling runs
                    });
                }
            })
            .catch(() => {
                // LND may be stopped during wallet switch
            });
    };
